    ...
    fun addMaskToEditText(editText: EditText, mask: String) {
        editText.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable) {
                ...
                val newText = s.toString().replace("[^\\d]".toRegex(), "")
                val formattedText = StringBuilder()
                var cleanTextIndex = 0

                for (i in mask.indices) {
                    if (cleanTextIndex >= newText.length) {
                        break
                    }

                    if (mask[i] == '#') {
                        formattedText.append(newText[cleanTextIndex])
                        cleanTextIndex++
                    } else {
                ...
                editText.setSelection(formattedText.length.coerceAtMost(mask.length))
                isUpdating = false
            }
        })
    }
